...
const notifier=({ title, subtitle, text, className, note, action, link, Style, intitle, children })=>{
  ...
  return (
    <Fragment>
      {!Notifier && <div className={` ${className} notifier d-i-f m-w-80 `} Style={` ${Style}`}>
      ...
      <div className="w-a h-a h-pr-fl.ma info p-20px">
      <div className="w-a h-a h-pr-fl.ma info p-20px"><div  className="w-and-h-40px circle h-e c-p" Style="border: 1px solid; border-color: white;centered;"><button  onClick={handleClose} type="button" className="centered f-f-gilroy" Style="color:white;background-color:transparent;">X</button>  </div> </div> 
      <div><button type="button"   className="h-pr-fl-ma b-s-b-b w-100 h-a f-f-Gilroy  m-t-60px info hover-underline-animation" Style="font-size: 15px; line-height: 15px; color: white;" onClick={handleClose} value={cookie.userName} onChange={handleChange}>Aceptar</button></div> 
      <div><button  type="button"  className="h-pr-fl-ma b-s-b-b w-100 h-a f-f-Gilroy  m-t-60px info hover-underline-animation" Style="font-size: 15px; line-height: 15px; color: white;" onClick={handleClose}  value={cookie.userName} onChange={handleRemove}>Rechazar</button></div> 
      </div>
      </div>
}
